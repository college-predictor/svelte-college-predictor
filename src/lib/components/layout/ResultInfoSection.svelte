<script lang="ts">
    ...
    // Define a type for the yearData structure
    interface YearData {
      tableData: { [key: string]: string | number }[];
      note?: string;
    }

    // Define a type for the services data structure
    interface Service {
      title: string;
      description: string;
      icon: any;
    ...
    // Update the services array to use the Service type
  ...
  <section class="bg-gradient-to-br from-slate-800 to-slate-700 py-16 md:py-10 relative overflow-hidden">
    ...
    <div class="w-full max-w-[1440px] mx-auto px-3 sm:px-4 md:px-6 text-center relative z-10">
      <div class="max-w-3xl mx-auto mb-3">
        <h4 class="text-2xl md:text-3xl font-extrabold mb-6 bg-clip-text text-transparent bg-gradient-to-r from-teal-400 to-sky-500">Result Information</h4>
      ...
      <div class="mt-5 md:mt-5 bg-white/80 backdrop-blur-sm rounded-xl shadow-lg w-full max-w-[1800px] mx-auto border border-teal-100 overflow-hidden">
        ...
        <div class="flex overflow-x-auto scrollbar-hide border-b border-teal-100 bg-teal-50/50">
        ...
        {#if services[activeTab]?.data?.yearData}
          <div class="flex overflow-x-auto scrollbar-hide border-b border-teal-100 bg-white">
